                        ...
                        tabPanel('PCA',
                                 sidebarLayout(
                                   sidebarPanel(
                                     ...
                                     selectInput('pca_x', 'x axis',choices = c('PC1'=1, 'PC2'=2, 'PC3'=3, 'PC4'=4)),
                                     uiOutput('pca_y')
                                     ),
...
server <- function(input, output,session) {
  ...
  output$pca_y <- renderUI({
    ...
    selectInput("pca_y", "y axis", choices = setNames(pca_y_choices, paste0("PC", pca_y_choices)))
           })
  
  
  
  output$pcaPlot <- renderPlot({
    g <- ggbiplot(ir.pca(), choices = c(as.numeric(input$pca_x), as.numeric(input$pca_y)),obs.scale = 1, var.scale = 1, groups = ir.species(), ellipse = TRUE) 
