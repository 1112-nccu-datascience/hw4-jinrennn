...
ui <- fluidPage(
  ...
  tags$head(
    ...
    titlePanel(tags$div(style = "color: #F8F8F8;","NCCU_DS2023_111753124")),
    navbarPage('',
      tabPanel('PCA',        
               tabsetPanel(
                 tabPanel('PCA',
    ...
    output$pca_y <- renderUI({
      if(input$pca_x ==1){
        selectInput("pca_y", 'y axis',
                    choices = c('PC2'=2, 'PC3'=3, 'PC4'=4) )
        } else if (input$pca_x ==2){
          selectInput("pca_y", 'y axis',
                      choices = c('PC1'=1, 'PC3'=3, 'PC4'=4) )
        }else if (input$pca_x ==3){
          selectInput("pca_y", 'y axis',
                      choices = c('PC1'=1, 'PC2'=2, 'PC4'=4) )
        }else{
          selectInput("pca_y", 'y axis',
